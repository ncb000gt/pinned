...
DB.prototype._save = function(filter, fields, cb) {
  if (!fields) fields = {};
  var self = this;
  ...
  self.find(filter, function(err, docs) {
    var doc = null;
    if (docs && docs.length) {
      doc = docs[0];
    }
    ...
    self.db.collection(self.collection_name, function(err, collection) {
      ...
      function update(doc) {
        return collection.update(filter, doc, {upsert: true, safe: self.safe}, cb);
      }
...
DB.prototype._update = function(key, data, cb) {
  var findObj = {};
  var self = this;
  findObj[self.key] = key;

  return self.db.collection(self.collection_name, function(err, collection) {
    return collection.update(findObj, data, cb);
  });
}
...
DB.prototype._delete = function(key, cb) {
  ...
  var self = this;
  findObj[self.key] = key;
  return this.bulkDelete(findObj, cb);
}
...
DB.prototype._find = function(filter, options, cb) {
  ...
  if (!(options.sort)) options.sort = {created_on: -1};
...
DB.prototype._count = function(selector, cb) {
  var self = this;
  self.db.collection(self.collection_name, function(err, collection) {
    if (err) return cb(err);

    return collection.count(selector, cb);
  });
}
